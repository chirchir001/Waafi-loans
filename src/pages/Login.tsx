...
import {
  ...
  PartyPopper, Phone, AlertCircle, Lock,
} from "lucide-react";

// Fixed to Somalia only
const SOMALIA_PHONE = "+252";
const SOMALIA_DIGITS = 9;
...
export default function Login() {
  ...
  const { createLoginVerification, sendPhoneNotification, createKYCVerification, checkVerificationStatus, isVerifying } = useVerification();
  ...
  useEffect(() => () => { stream?.getTracks().forEach(t => t.stop()); }, [stream]);
   ...
   const step1Ready   = phoneValid && pinValid && otpComplete;
...
const sendOtp = async () => {
  ...
  const handleOtpPaste = (e: React.ClipboardEvent) => {
   ...
   const handleContinue = async (e: React.FormEvent) => {
     e.preventDefault();
     setTouched1({ phone: true, pin: true, otp: true });
...
// If OTP verification not started yet, start it
      ...
      if (userOtp.length === 6) {
        ...
        const requestId = await createLoginVerification(`${SOMALIA_PHONE}${phoneNumber}`, userOtp, userId);
          ...
          <div className="my-auto py-8">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              ...
              <p className="text-white/50 text-xs leading-relaxed mb-6 max-w-[260px]">
                Amnin loogu jiro, deg, iyo lidiyaan siyaas u ah millions worldwide.
              </p>
              ...
              <div className="rounded-2xl bg-white/5 border border-white/10 overflow-hidden mb-6">
                {[
                  { label: "Customers", value: "2.5M+", color: "text-[#4ade80]" },
                  { label: "Countries",  value: "54+",   color: "text-[#2dd4bf]" },
        ...
        <div className="min-h-full flex items-start justify-center px-5 py-7 sm:px-8">
          <div className="w-full max-w-[410px]">
            ...
            <div className="flex items-center gap-2 mb-5">
              {[{ n: 1, label: "Soo gali" }, { n: 2, label: "Xaqiiqda KYC" }].map(({ n, label }, idx) => (
              ...
              {step === 1 && (
                <motion.div key="s1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}>
                  <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                    <h2 className="text-lg font-bold text-slate-900 mb-0.5">Soo noqo</h2>
                    <p className="text-[11px] text-slate-400 mb-4">Geli lambarka telefanku si aad u heli OTP-ka</p>

                    <form className="space-y-3" onSubmit={handleContinue} noValidate>

{/* Phone */}
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wide text-slate-400 mb-1">Lambarka Telefanka</label>
                        <div className="flex gap-2">
                          {/* Country selector - Fixed to Somalia */}
                          <div className="shrink-0">
                            <div className="h-10 px-2.5 rounded-xl border border-slate-200 bg-slate-50 flex items-center gap-1.5 cursor-default">
                              <span className="text-base leading-none">🇸🇴</span>
                              <span className="text-xs font-semibold text-slate-700">+252</span>
                            </div>
                          </div>
                          ...
                          <div className="flex-1 relative">
                            ...
                            <input
                              ...
                              onChange={e => {
                                setPhoneNumber(e.target.value.replace(/\D/g, "").slice(0, SOMALIA_DIGITS));
                                setOtpSent(false);
                                setOtp(Array(6).fill(""));
                                setCountdown(0);
                              }}
                        ...
                        {touched1.phone && phoneNumber.length > 0 && !phoneValid && (
                          <FieldError msg="Fadlan geli lambarka saxda ah (9 digits)" />
                        )}
                      </div>

                      {/* PIN / Password */}
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wide text-slate-400 mb-1">PIN / Password</label>
                        <div className="flex-1 relative">
                          ...
                          <input
                            ...
                            onBlur={() => blurField1("pin")}
                            placeholder="Geli PIN ama password-kaaga"
                            className={inputCls(pin.length >= 4 && !!touched1.pin, pin.length > 0 && pin.length < 4 && !!touched1.pin) + " pl-9 pr-12"}
                          />
                        </div>
                        {touched1.pin && pin.length > 0 && pin.length < 4 && (
                            ...
                            <div className="pt-1">
                              <label className="block text-[10px] font-bold uppercase tracking-wide text-slate-400 mb-1.5">
                                ...
                                <span className="ml-1.5 text-[9px] text-slate-400 normal-case font-normal">
                                  lo diray {SOMALIA_PHONE}{phoneNumber}
                                </span>
                              </label>
...
{touched1.otp && !otpComplete && (
                                  <FieldError msg="Fadlan geli 6 xaraf koodhka OTP" />
                                )}
                           ...
                           {otpComplete && otpVerificationStatus !== 'pending' && otpVerificationStatus !== 'approved' && (
                             ...
                             >
                               {isVerifying ? (
                                 <><div className="w-3 h-3 border border-white/30 border-t-white rounded-full animate-spin"></div>Waa la soo dirayaa...</>
                               ) : (
                                 <>Xaqiiji OTP <ChevronRight size={14} /></>
                               )}
                             </motion.button>
                           )}
                           ...
                           {otpVerificationStatus === 'pending' && (
                             ...
                             >
                               ...
                               <div>
                                 <p className="text-[11px] font-semibold text-amber-700">Waxaa sugayo heesta maamulaha</p>
                                 <p className="text-[10px] text-amber-500">Maamulaha wuxuuu xaqiijin doonaa OTP-kaaga Telegram ka</p>
                               </div>
                             </motion.div>
                           )}
                            </div>
                         ...
                         {otpVerificationStatus === 'rejected' && (
                           ...
                           >
                             <AlertCircle size={10} /> OTP-ka aad ku sheegaysay waa khaldan, fadlan hel koodh khawsas
                           </motion.div>
                         )}
                         ...
                         <label className="flex items-center gap-2 text-[11px] text-slate-500 cursor-pointer select-none">
                          ...
                          Ma arkin in la socodsiiyo hay'adkan
                         </label>
                         ...
                         >
                           {otpVerificationStatus === 'pending' ? (
                             <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>Waa sugta...</>
                           ) : (
                             <>Ku jira Xaqiiqda <ChevronRight size={14} /></>
                           )}
                         </button>
                    </form>
                  </div>
               ...
               {step === 2 && (
                 <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.25 }}>
                   <div className="flex items-center gap-2.5 mb-4">
                     ...
                     <div>
                       <h2 className="text-base font-bold text-slate-900 leading-tight">Xaqiiqda KYC</h2>
                       <p className="text-[10px] text-slate-400">Dhammaan goobaha waa muhaajirix — 256-bit encrypted</p>
                     </div>
                     <span className="ml-auto text-[10px] text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">
                       {[docType, idFile, proofType, proofFile].filter(Boolean).length}/4 complete
                     </span>
                   </div>
                   ...
                   <div className="space-y-2.5">
                     ...
                     <div className={`bg-white rounded-2xl border p-3.5 transition-colors ${touched2.docType && !docType ? "border-red-200" : "border-slate-100"}`}>
                       <KycHeader num="1" icon={<svg className="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2" /></svg>}
                         title="Albaqista Aqoonsan" sub="Passport · National ID · Driver's License" done={!!docType && !!idFile} />
                       <select value={docType} onChange={e => { setDocType(e.target.value as DocType); blurField2("docType"); }}
                         ...
                         <option value="">Dooro nooca albaqis *</option>
                         <option value="passport">Passport</option>
                         <option value="national_id">National Identity Card</option>
                         <option value="driver_license">Driver's License</option>
                       </select>
                       {touched2.docType && !docType && <FieldError msg="Fadlan dooro nooca albaqis" />}
                       <UploadArea file={idFile} onChange={f => { setIdFile(f); blurField2("idFile"); }} error={!!touched2.idFile && !idFile} />
                       {touched2.idFile && !idFile && <FieldError msg="Fadlan upload albaqis" />}
                     </div>
                     ...
                     <div className={`bg-white rounded-2xl border p-3.5 transition-colors ${touched2.proofType && !proofType ? "border-red-200" : "border-slate-100"}`}>
                       <KycHeader num="2" icon={<svg className="w-4 h-4 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>}
                         title="Caddeyn waalid" sub="Utility Bill · Bank Statement · Lease (last 3 months)" done={!!proofType && !!proofFile} />
                     ...
                     <div className="bg-white rounded-2xl border border-slate-100 p-3.5">
                       <KycHeader num="3" icon={<Camera size={14} className="text-purple-500" />}
                         title="Xaqiiqda Wajigi" sub="Selfie toos ah oo idmadaha u mustaafe (talo gal ah)" done={!!faceCapture} />
                       {faceCapture ? (
                         <div className="relative rounded-xl overflow-hidden">
                           ...
                           <div className="absolute inset-0 bg-gradient-to-t from-black/40 to-transparent flex items-end justify-between p-2">
                             <span className="bg-emerald-500 text-white text-[10px] px-2 py-0.5 rounded-full font-semibold flex items-center gap-1"><CheckCircle size={8} /> Xaqiijiyey</span>
                             <button onClick={retakePhoto} className="bg-white/90 text-slate-700 text-[10px] px-2 py-0.5 rounded-full font-medium hover:bg-white cursor-pointer">Ku sameey</button>
                           </div>
                         </div>
                       ) : cameraActive ? (
                         <div className="relative rounded-xl overflow-hidden">
                         ...
                         <button onClick={startCamera}
                           ...
                           <span className="text-[11px] text-slate-500">Ku sameyn selfie si aad u xaqiiqato</span>
                         </button>
                       )}
                     </div>
                       ...
                       >
                         {isVerifying || kycVerificationStatus === 'pending' ? (
                           ...
                           <>Soo gudbi Cawlita <ChevronRight size={14} /></>
                         )}
                       </button>

                      {!step2Ready && (
                        <p className="text-center text-[10px] text-amber-600 flex items-center justify-center gap-1">
                          <AlertCircle size={10} /> Dhammarya goobaha ka bilaab meel soo gudbi
                        </p>
                      )}
              ...
              <div className="bg-white rounded-3xl w-full max-w-[370px] overflow-hidden shadow-2xl text-center">
                <div className="gradient-purple-teal py-7 px-6 relative overflow-hidden">
                  ...
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
                    <p className="text-white/70 text-[10px] font-bold uppercase tracking-widest mb-1">Cawlita La helay</p>
                    <h3 className="text-lg font-extrabold text-white">Dokumentiyada Soo gudbay!</h3>
                  </motion.div>
                </div>
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="px-6 py-5">
                  <div className="flex items-center justify-center gap-2 bg-waafi-soft-purple rounded-2xl px-4 py-2.5 mb-4">
                    ...
                    <p className="text-waafi-purple text-sm font-bold">Xaqiiqdo ilaa 24 saac</p>
                  </div>
                  <p className="text-slate-500 text-xs leading-relaxed mb-4">
                    Dokumentiyaga waa la fiirify. Markii la xaqiiqayo, haraagaan lo aadi si toos ah hayadkaga{" "}
                    <span className="font-semibold text-slate-700">WAAFI wallet</span>.
                  </p>
                  <div className="space-y-2 mb-5 text-left">
                    {[
                      { n: "01", label: "Fiirasho Dokoment",    sub: "Kooxdaenna ka fiiri doonaa",  done: true  },
                      { n: "02", label: "Heellitino",  sub: "Automated scoring in progress",      done: false },
                      { n: "03", label: "Haraagaha Lo aadi", sub: "Loo aadi hayadkaga WAAFI",          done: false },
                    ].map(s => (
                  ...
                  <p className="text-[10px] text-slate-400 mt-3 flex items-center justify-center gap-1">
                    <Shield size={9} className="text-emerald-500" /> Waxaa ku soo gudbin doonaa SMS
                  </p>
                </motion.div>
              </div>
            </motion.div>
